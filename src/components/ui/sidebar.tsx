...
>(({ className, showIcon = false, ...props }, ref) => {
  // Derived from useId, NOT Math.random(). This app is server-rendered, and
  // useMemo runs on the server as well as the client — so a random width was
  // computed twice, differed every time, and produced a guaranteed hydration
  // mismatch ("the server rendered HTML didn't match the client") on any page
  // that rendered a sidebar skeleton. useId is stable across both passes, so the
  // widths still vary between rows but agree between server and client.
