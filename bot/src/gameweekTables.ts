...
async function getTableChannelId(
  ...
  return data?.table_channel_id ?? null;
}
...
function buildRows(
  ...
  const rows = teams.map((team) => {
    ...
    return {
      position: 0,
      team,
      played: formatNumber(standing?.played),
      won: formatNumber(standing?.won),
      drawn: formatNumber(standing?.drawn),
      lost: formatNumber(standing?.lost),
      goalsFor: formatNumber(
        standing?.goals_for,
      ),
      goalsAgainst: formatNumber(
        standing?.goals_against,
      ),
      goalDifference: formatNumber(
        standing?.goal_difference,
      ),
      points: formatNumber(standing?.points),
    };
  });
...
async function buildTableSvg(
  ...
  const svgRows = rows
    .map((row, index) => {
      const y =
        ...
        columnHeaderHeight +
        index * rowHeight;
      const logo = logoMap.get(row.team.id);
      const logoMarkup = logo
        ...
        `;
      const teamName = escapeXml(
        truncate(row.team.name, 27),
      );
      return `
        <rect
          x="40"
  ...
  return `
    ...
    >
      ...
      <text x="${pX}" y="${headerHeight + 45}" text-anchor="middle" font-size="17" font-weight="900" fill="#71717a">P</text>
      <text x="${wX}" y="${headerHeight + 45}" text-anchor="middle" font-size="17" font-weight="900" fill="#71717a">W</text>
      <text x="${dX}" y="${headerHeight + 45}" text-anchor="middle" font-size="17" font-weight="900" fill="#71717a">D</text>
      <text x="${lX}" y="${headerHeight + 45}" text-anchor="middle" font-size="17" font-weight="900" fill="#71717a">L</text>
      <text x="${gfX}" y="${headerHeight + 45}" text-anchor="middle" font-size="17" font-weight="900" fill="#71717a">GF</text>
      <text x="${gaX}" y="${headerHeight + 45}" text-anchor="middle" font-size="17" font-weight="900" fill="#71717a">GA</text>
      <text x="${gdX}" y="${headerHeight + 45}" text-anchor="middle" font-size="17" font-weight="900" fill="#71717a">GD</text>
      <text x="${ptsX}" y="${headerHeight + 45}" text-anchor="middle" font-size="17" font-weight="900" fill="#ffffff">PTS</text>
      ${svgRows}
      <text
        x="${left}"
        y="${height - 25}"
...
async function postTable(
  ...
  const existing = await hasTablePost(
    division.id,
    gameweekNumber,
  );
  if (existing) {
  ...
  const png = await generateTablePng(
    league,
    division,
    gameweekNumber,
    rows,
  ...
  const text =
    gameweekNumber === 0
      ? `🏆 **${division.name} • GW0 Table**\nThe opening standings for **${league.name}** are here.`
      : `🏆 **${division.name} • GW${gameweekNumber} Table**\nThe standings have been updated after Gameweek ${gameweekNumber}.`;
  let message;
  try {
    message = await (
...
export function startGameweekTableWatcher(
  ...
  void checkGameweekTables(client);
  setInterval(() => {
    void checkGameweekTables(client);
  }, WATCH_INTERVAL_MS);
}
