...
/*
 * Discord signed attachment URLs expire.
 *
 * We still try the stored URL, because a currently-valid
 * Discord URL can work. If it has expired, we fall back
 * to a generated team badge instead of losing the row.
 */
async function downloadLogo(
  ...
  try {
    ...
    if (!response.ok) {
      console.log(
        `[GameweekTables] Logo request failed for "${team.name}" with status ${response.status}.`,
      );
      return null;
    }
    const contentType =
    ...
    if (
      ...
      console.log(
        `[GameweekTables] Logo for "${team.name}" is not an image.`,
      );
      return null;
    }
    const buffer =
...
async function generateTablePng(
  ...
  /*
   * We explicitly load the directory as well as the font file.
   *
   * The SVG uses generic "sans-serif".
   * Resvg maps that generic family to DejaVu Sans below.
   */
  const fontDirectory =
    dirname(
      FONT_PATH,
...
async function gameweekIsComplete(
  ...
  /*
   * fixtures uses "gameweek", not "gameweek_number".
   */
  const { data, error } =
    await supabase
      .from("fixtures")
...
async function checkDivision(
  ...
  /*
   * gameweeks.number is the actual DB column.
   */
  const {
    data: gameweeks,
    error,
  ...
  for (const gameweek of gameweeks) {
    ...
    if (!complete) {
      ...
      /*
       * Later gameweeks cannot be complete if this one isn't.
       */
      break;
    }
    /*
     * Re-read standings after the completed gameweek.
     * This means the table graphic uses the latest DB values.
     */
